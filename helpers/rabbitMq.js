...
startAll = () => {
  ...
  consume("session_end", async ({ positionId }) => {
    await Session.updateMany({ positionId }, { $set: { inProgress: false, updatedAt: Date.now() } })
    console.log(`RabbitMQ: Session closed at position ${positionId}`)
  })

  consume("recognition_feedback", async ({ recognitionId, brand, color, size, model }) => {
    try {
      ...
      console.error(`RabbitMQ: Receive feedback for ${recognitionId} but it was not found in DB`)
    }
  })
